...
// +----------------------------------------------------------------------
// |  Validator 验证器 By:pushaowei@sporte.cn
// +----------------------------------------------------------------------
...
// |    optional - Value does not need to be included in data array.
// +----------------------------------------------------------------------
/**
 * Validator 验证器
 *
 ...
 * @license sporte.cn
 * @copyright 2016-12-07 18:47:41   体创(上海)云科技有限公司
 */
class Validator
{
    /**
    ...
    /**
     * [validatePhone 检测是否为手机]
     * @author         ShaoWeiPu <pushaowei@sporte.cn>
     * @CreateTime    2016-12-14T11:02:13+0800
    ...
    /**
     * [validateIdCard 检测身份证]
     * @author         ShaoWeiPu <pushaowei@sporte.cn>
     * @CreateTime    2016-12-14T10:44:45+0800
     * @return                              [type] [description]
     */
    protected function validateIdcard($field, $value)
    {
        if(strlen($value)==18){
     ...
     * @return bool
     */
    protected function validateRequiredIf($field, $value,$params)
    {

        if( $this->_fields[$params[0]] == $params[1]){
    ...
    protected function validateDateBefore($field, $value, $params)
    {
        ...
        if(isset($this->_fields[$params[0]])){
            $params[0] = $this->_fields[$params[0]];
        }
    ...
    /**
     * [validateIntegerForStr description]
     * @param  [type] $field [description]
     * @param  [type] $value [description]
     * @return [type]        [description]
     */
    protected function validateIntegerForList($field, $value){
    ...
    public function errorMsgList($value='')
    {
        $list = [];
    ...
    public function errorCode($field,$rule){

        array_push($this->_errorCode, "$field.$rule");
    }

    public function errorCodeList($field = null)
    {
     ...
     * @return boolean
     */
    public function validate($message_list = array())
    {
        foreach ($this->_validations as $v) {
            foreach ($v['fields'] as $field) {
                ...
                if (!in_array($v['rule'],['required','requiredIf']) && !$this->hasRule('required', $field)  && (! isset($values) || $values === '' || ($multiple && count($values) == 0))) {
                    continue;
                }
                ...
                if (!$result) {
                    ...
                    if(isset( $message_list[$field.'.'.$v['rule']] )){
                         $v['message'] = $message_list[$field.'.'.$v['rule']];
                    }
                    $this->error($field, $v['message'], $v['params']);
                    $this->errorCode($field,$v['rule']);
